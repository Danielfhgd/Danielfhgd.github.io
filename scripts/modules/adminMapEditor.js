...
class AdminMapEditor {
    ...
    async confirmarAgregar() {
        ...
        // Verificar estructura (algunos JSONs tienen array en lugar de objeto para zonas)
        // Convertiremos arrays a objetos para facilitar la edición
        if (datosMapa[piso][sector] && Array.isArray(datosMapa[piso][sector])) {
            // Convertir Array -> Objeto (para poder editar/agregar fácilmente)
            const obj = {};
            datosMapa[piso][sector].forEach(item => {
                const key = Object.keys(item)[0];
                obj[key] = item[key];
            });
            datosMapa[piso][sector] = obj;
        } else if (!datosMapa[piso][sector]) {
            datosMapa[piso][sector] = {};
        }

        // Determinar claves internas del sector (dataUBV usa "ala-este", el select usa "Ala_Este")
        // Mapeo simple:
        let targetSector = sector;
        if (sector === 'Ala_Este') targetSector = 'ala-este';
        if (sector === 'Ala_Oeste') targetSector = 'ala-oeste';
        if (sector === 'Centro') targetSector = 'centro';

        // Asegurar que exista con la clave correcta del JSON
        if (!datosMapa[piso][targetSector]) {
            // Si no existe, ver si existe con otras variantes
            if (datosMapa[piso]['ala-este']) targetSector = 'ala-este';
            else if (datosMapa[piso]['ala-oeste']) targetSector = 'ala-oeste';
            else if (datosMapa[piso]['centro']) targetSector = 'centro';
            else datosMapa[piso][targetSector] = {}; // Crear si no existe
        }

        // Si es array, convertirlo AHORA
        if (Array.isArray(datosMapa[piso][targetSector])) {
            const obj = {};
        ...
        if (confirm("Punto guardado. ¿Recargar mapa para ver el nuevo punto?")) {
            location.reload();
        }
    }
