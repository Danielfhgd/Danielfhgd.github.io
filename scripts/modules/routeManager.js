...
class RouteManager {
    ...
    async calculateRoute(originId, destId) {
        ...
        if (diffFloors) {
            ...
            const findStairCoords = (piso, zona, nombre) => {
                let res = this.getLocationCoords(piso, zona, nombre);
                if (!res) res = this.getLocationCoords(piso, zona.toLowerCase(), nombre);
                if (!res) res = this.getLocationCoords(piso, zona.replace('_', '-'), nombre);
                if (!res) res = this.getLocationCoords(piso, 'Ala_Este', nombre);
                if (!res) res = this.getLocationCoords(piso, 'Ala_Oeste', nombre);
                if (!res) res = this.getLocationCoords(piso, 'Centro', nombre);
                return res;
            };

            let escIzq = findStairCoords(pisoO, 'Ala_Oeste', 'escalera_izquierda');
            let escDer = findStairCoords(pisoO, 'Ala_Este', 'escalera_derecha');

            if (!escIzq && !escDer) {
                this.statusBar.innerText = "⚠️ No se encontraron escaleras (revise nombres: escalera_izquierda/derecha).";
                this.drawRoute(pA, pB);
                return;
            }

            let escIzqD = findStairCoords(pisoD, 'Ala_Oeste', 'escalera_izquierda');
            let escDerD = findStairCoords(pisoD, 'Ala_Este', 'escalera_derecha');

            // Calcular distancias (Geometría Euclídea simple)
            ...
            // Lógica corregida: Solo podemos evaluar una escalera si existe en AMBOS pisos
            if (escIzq && escIzqD) {
                const e1 = toObj(escIzq);
                const e2 = toObj(escIzqD);
                costoIzq = dist(oObj, e1) + dist(dObj, e2);
            }
            if (escDer && escDerD) {
                const e1 = toObj(escDer);
                const e2 = toObj(escDerD);
                costoDer = dist(oObj, e1) + dist(dObj, e2);
            }
            ...
            // Simplificado: solo "Escalera Izquierda" o "Escalera Derecha"
            ...
            // GUARDAR ESTADO DE LA RUTA GLOBAL (Para dibujar cuando cambie el piso)
            ...
            // Helper para formatear el destino
            // Si empieza con "Piso", usar "al" (al Piso 1), si no (Sótano), "al" o "a" según corresponda.
            // Simplemente usaremos "ir a [Destino]" que funciona bien para "ir a Piso 1" y "ir a Sótano".
            const formatDest = (name) => {
                if (name.toLowerCase().startsWith('piso')) return `al ${name}`;
                return `al ${name}`; // "al Sótano"
            };
